                         ...
                         tcx: &ty::ctxt<'tcx>) -> &'tcx ty::ADTDef_<'tcx, 'tcx>
{
    ...
    fn get_struct_variant<'tcx>(intr: &IdentInterner,
        ...
        let did = translated_def_id(cdata, doc);
        ty::VariantDef_ {
            did: did,
    ...
    fn fill_field_types<'tcx>(cdata: Cmd,
                              adt: &'tcx ty::ADTDef_<'tcx, 'tcx>,
                              tcx: &ty::ctxt<'tcx>) {
    }
    ...
    let (kind, variants) = match item_family(doc) {
        ...
        Struct => (ty::ADTKind::Struct,
                   vec![get_struct_variant(intr, cdata, doc, tcx)]),
        _ => tcx.sess.bug("get_adt_def called on a non-ADT")
    };

    let adt = tcx.intern_adt_def(ast::DefId {
        krate: cdata.cnum,
        node: item_id
    }, kind, variants);
    ...
    fill_field_types(cdata, adt, tcx);
