...
use rustc::ty::{self, FnOutput, TyCtxt, ParameterEnvironment};
use rustc::mir::repr::*;
...
impl<T> Index<Location> for LocationMap<T> {
    ...
    fn index(&self, index: Location) -> &Self::Output {
        &self.map[index.block][index.index]
    }
}

impl<T> IndexMut<Location> for LocationMap<T> {
    fn index_mut(&mut self, index: Location) -> &mut Self::Output {
        &mut self.map[index.block][index.index]
    }
}
...
impl<'a, 'tcx> MoveDataBuilder<'a, 'tcx> {
                                ...
                                -> Result<MovePathIndex, ErrorReported>
    {
        ...
        let lv_ty = self.mir.lvalue_ty(self.tcx, &proj.base).to_ty(self.tcx);
        match lv_ty.sty {
...
fn gather_moves<'a, 'tcx>(mir: &Mir<'tcx>,
    ...
    for (bb, block) in mir.basic_blocks().iter_enumerated() {
        ...
        let terminator_loc = Location { block: bb, index: block.statements.len() };
...
impl<'a, 'tcx> MoveDataBuilder<'a, 'tcx> {
    ...
    fn gather_terminator(&mut self, loc: Location, term: &Terminator<'tcx>) {
        ...
        match term.kind {
            ...
            TerminatorKind::Return => {
                if let FnOutput::FnConverging(_) = self.mir.return_ty {
                    self.gather_move(loc, &Lvalue::ReturnPointer);
                } else {
                    debug!("gather_terminator: assuming function diverges");
                }
            }
    ...
    fn gather_move(&mut self, loc: Location, lval: &Lvalue<'tcx>) {
        ...
        let lv_ty = self.mir.lvalue_ty(self.tcx, lval).to_ty(self.tcx);
